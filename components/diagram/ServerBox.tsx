...
export default async function ServerBox({ children, hint }: ServerBoxProps ) {
    ...
    const user: User = await fetch(`https://jsonplaceholder.typicode.com/users/${randomNumber}`, { cache: "no-store" })
        .then(data => data.json());
    ...
    return (
        <div className={"bg-orange-600 p-5 border-2 border-black"}>
            ...
            <div className={"border-gray-700 border-2 w-fit m-auto my-5"}>
                <div className={"text-2xl text-gray-300 m-2"}>{user.name}</div>
                <div className={"text-2xl text-gray-300 m-2"}>{user.email}</div>
                <div className={"text-2xl text-gray-300 m-2"}>{user.phone}</div>
            </div>
